...
def main():
    ...
    wandb_logger = WandbLogger(project="Baseline", name="L1_regularization")
    train = pl.Trainer(logger=wandb_logger, max_epochs=150, log_every_n_steps=1)
    model = Resnet(l1_strength=1e-5)
    train.fit(model=model, train_dataloaders=train_loader, val_dataloaders=valid_loader)
